...
const copilotText = copilot.replace(/\s+/g, " ");
...
describe("NSOS Copilot setup agent interface", () => {
  it("hands owner/admin setup goals into the direct Automation Desk instead of trapping users in a fragmented preparation panel", () => {
    expect(copilotText).toContain('onNavigate("automation")');
    expect(copilotText).toContain("nsos-automation-desk-request");
    expect(copilotText).toContain(
  ...
  it("presents the NSOS Guide as a prompt-led handoff into a reviewable job and remediates priority static workspace controls", () => {
    expect(copilotText).toContain("NSOS Guide");
    expect(copilotText).toContain(
      "Describe what you want to build or operate."
    );
    expect(copilotText).toContain("Help me build or refine my institution.");
    expect(copilotText).toContain("Review preparation");
    expect(copilotText).toContain(
      "Setup goals can continue in Automation Desk as a visible reviewed job"
    );
