...
const dashboard = readFileSync(
  new URL(
    "../client/src/components/OwnerSetupManagementDashboard.tsx",
    import.meta.url
  ),
  "utf8"
);
...
describe("owner setup management dashboard", () => {
  it("keeps setup approvals in an owner-only workspace route", () => {
    ...
    expect(home).toMatch(
      /if \(view === "setup-management"\) return role === "owner" \?\s*\(?\s*<OwnerSetupManagementDashboard schoolId=\{schoolId\}/
    );
  });
  ...
  it("shows both pending queues and retains explicit confirmation for each sensitive action", () => {
    ...
    expect(dashboard).toContain(
      "I give final owner approval to activate this fee structure."
    );
    ...
    expect(dashboard).toContain(
      "approvalNote: feeApprovalNote.trim() || undefined"
    );
  ...
  it("does not imply automatic invitation delivery or finance activation", () => {
    ...
    expect(dashboard).toContain(
      "This dashboard never invents staff identities"
    );
