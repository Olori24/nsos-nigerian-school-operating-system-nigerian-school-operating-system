...
6. **Frontend delivery still needs optimisation.** Route-level lazy loading reduced the measured primary Vite entry substantially, and deferred rich Copilot rendering reduced the emitted Mermaid chunk. The primary entry remains 662.00 kB before gzip, the lazy dashboard page chunk is 954.25 kB, and the Mermaid chunk remains 510.40 kB; all continue to trigger the build-size warning. Bundle analysis, further feature-level splitting, and representative mobile-network measurement should precede acquisition campaigns.
