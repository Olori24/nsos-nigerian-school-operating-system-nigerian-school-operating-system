...
import { BellRing, Copy, CreditCard, KeyRound, Loader2, LockKeyhole, PlugZap, Save, Send, ShieldCheck, Trash2 } from "lucide-react";
...
export function ProviderConfigurationCenter({ schoolId }: { schoolId: number }) {
  ...
  const automaticSmsDeliveryReady = Boolean(selected?.id && selectedProviderIsSaved && selected.status === "ready" && (provider === "twilio" || (provider === "termii" && selected.hasWebhookSecret)));
  const copyWebhookUrl = () => {
    if (!callbackUrl) return;
    navigator.clipboard.writeText(callbackUrl).then(() => toast.success("Webhook URL copied."), () => toast.error("Could not copy the webhook URL. Select and copy it manually."));
  };
