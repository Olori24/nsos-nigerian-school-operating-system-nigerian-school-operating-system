...
- [ ] Obtain separate approval and a disposable MySQL 8.4 environment to execute the repaired full migration chain after journal/column preflight; do not connect to an existing or live schema. The newly approved disposable rehearsal ran once and stopped safely at an additional out-of-scope MySQL-incompatible `CREATE INDEX IF NOT EXISTS` in migration `0003`; the partial target was read-only inventoried and fully destroyed. Full-chain validation remains blocked until a newly reviewed repair covers this and any further incompatibilities.
...
- [ ] Execute the owner-approved fresh MySQL 8.4 rehearsal using only the four-file artifact with SHA-256 `4104e083f3262d34de00e310ed74d47f13b772b0cff2b568d2bf2dc6464cb7d1`; require fresh-target preflight, full-chain and no-op evidence, then mandatory target teardown regardless of result.
