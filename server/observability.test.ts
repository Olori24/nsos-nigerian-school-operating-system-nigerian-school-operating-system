...
describe("production observability controls", () => {
  it("returns a minimal liveness payload without configuration, tenant, or infrastructure data", () => {
    expect(
      livenessResponse(() => new Date("2026-08-26T20:00:00.000Z"))
    ).toEqual({
      status: "ok",
      service: "nsos",
  ...
  it("uses a bounded correlation ID and never includes a query string in its request path", () => {
    ...
    expect(
      safeRequestPath({
        baseUrl: "/api",
        path: "/trpc/nsos.portal?token=secret",
      } as any)
    ).toBe("/api/trpc/nsos.portal");
  });

  it("classifies request outcomes using bounded, aggregation-safe categories", () => {
    expect(requestOutcomeForStatus(200)).toBe("success");
    expect(requestOutcomeForStatus(302)).toBe("redirect");
    expect(requestOutcomeForStatus(403)).toBe("client_error");
    expect(requestOutcomeForStatus(503)).toBe("server_error");
  });
  ...
  it("records only correlation, method, path, status, and duration on completion", () => {
    ...
    const request = {
      method: "POST",
      baseUrl: "/api",
      path: "/trpc/nsos.finance.save",
      get: vi.fn(() => "trace-12345678"),
    };
    const log = vi.fn();
    const next = vi.fn();
    let current = 100;
    requestObservabilityMiddleware({ now: () => current, log: log as any })(
      request as any,
      response as any,
      next
    );
  ...
  it("fails production startup only when a core runtime secret or connection setting is absent", () => {
    ...
    expect(
      ...
      "OAUTH_SERVER_URL is required in production.",
    ]);
  });
  ...
  it("returns an opaque correlated unexpected-error response without logging the error message", () => {
    ...
    unexpectedErrorObservabilityMiddleware({ log: log as any })(
      new Error("provider key secret"),
      request as any,
      response as any,
      vi.fn()
    );
    ...
    expect(log).toHaveBeenCalledWith("error", "http_request_unhandled", {
      requestId: "trace-12345678",
      method: "POST",
      path: "/api/auth",
      statusCode: 500,
