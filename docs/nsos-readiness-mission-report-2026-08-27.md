...
| Resend sender health | **Provider says verified; API reconciliation remains** | The owner-approved DomainKing correction makes `send.nsos.top` resolve publicly to the exact required target, `send.forge.rmta.net.` Resend Support subsequently said the domain is verified and ready to send. One sanitized authenticated read-only API check still returned HTTP 200 with `partially_failed`, sending enabled, receiving disabled, and no record details in the list payload. | Do not send email, enable inbound receiving, add mailboxes, alter recipients, or make another DNS change until the API state converges or Resend explains the discrepancy. |
