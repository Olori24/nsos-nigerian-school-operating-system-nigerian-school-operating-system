...
| Incomplete observability | The runtime bootstrap has correlation-safe request completion and unexpected-error events, but no histograms, database-pool telemetry, hosted metrics, or alert routing. | Retain the privacy-safe event boundary and add bounded external-dependency metrics plus documented operational dashboards/alerts. |
...
| Auth/provider error pipeline | Authentication uses provider deadlines and validated OAuth state. The Express fallback now emits an opaque correlation-safe unexpected-error event without error messages, credentials, tokens, keys, provider bodies, recipient data, or query strings. | Add externally retained metrics and authorised incident handling without broadening logged data. |
