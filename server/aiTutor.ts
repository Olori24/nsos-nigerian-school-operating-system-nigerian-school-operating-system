...
export async function generateSupervisedTutorResponse(input: { tutorName: string; subjectName: string; curriculumScope: string; allowedLevels: string[]; question: string; teachingStyle?: "balanced" | "step_by_step" | "worked_examples" | "concise_review" }) {
  ...
  const response = await invokeLLM({
    ...
    maxTokens: 800,
    messages: [
      { role: "system", content: "You are a supervised school study tutor, not a teacher of record or a replacement for adult supervision. Help only with the configured subject and curriculum scope. Explain concepts, give a short worked example when appropriate, and propose study steps. Do not conduct or grade an examination, make high-stakes academic decisions, provide answer keys for live tests, request or retain personal information, discuss medical/legal/financial matters, or give advice about personal safety, abuse, self-harm, sex, drugs, or relationships. Never claim qualifications or certainty. If the learner’s request is outside scope, needs a teacher’s judgment, might be a live graded assessment, or lacks essential context, set needsTeacherSupport true and explain in a neutral way that a supervising teacher should help. The explanation format may be set from aggregate rating categories only; never mention, infer, or reveal ratings, feedback comments, or learner data. Return only the requested JSON." },
      { role: "user", content: `Tutor: ${input.tutorName}. Subject: ${input.subjectName}. Approved curriculum scope: ${input.curriculumScope.slice(0, 3000)}. Intended levels: ${input.allowedLevels.join(", ")}. Explanation format: ${teachingStyleInstruction(input.teachingStyle ?? "balanced")}. Learner question: ${question}` },
    ],
    outputSchema: {
...
function teachingStyleInstruction(style: "balanced" | "step_by_step" | "worked_examples" | "concise_review") {
  if (style === "step_by_step") return "Use numbered, small steps. Check understanding with one short reflection question; do not turn it into a graded assessment.";
  if (style === "worked_examples") return "Use one short, generic worked example before the study steps. Do not provide answers to a live or graded assessment.";
  if (style === "concise_review") return "Give a compact recap with the key idea, one illustrative point, and practical next steps.";
  return "Use a balanced explanation with a short example when useful and clear study steps.";
}
