...
NSOS now supports schools, vocational training centres, coaching centres, online-training providers, hybrid operators, and corporate academies through a common learning-operations model. Owners and administrators can manage programmes, cohorts, instructor assignments, internal programme fees, attendance, curriculum pathways, modules, milestones, materials, reviewed learner progress, and controlled milestone-evidence review. Each operator can use an appropriate internal structure—from a school learning sequence to a vocational competency pathway, coaching plan, online learning path, hybrid learning path, workplace capability path, or custom learning path—without changing existing school subjects, schemes of work, lessons, assessments, or results. Corporate academies can use internal role-relevant capability pathways, practice, simulations, and manager-reviewed development checkpoints; this does not make employment, performance, compliance, qualification, accreditation, or public claims. A linked learner may submit a concise internal note for an active milestone; owners, administrators, and actively assigned instructors can review only the records their tenant role permits. Evidence acceptance does not change progress, completion, grades, certificates, private issuer records, payments, messages, accounts, or public records. Course designers can choose a small curated evidence library, record tenant-owned planning references, and specify a transparent pace/support/practice profile for an editable internal draft. Institutions can separately create and activate a private issuer policy, then issue a private record only after human-confirmed programme completion, reviewed active milestones, evidence summary, and a final confirmation. Learners see only their own permitted programme state. NSOS does not claim accreditation or curriculum approval, automatically complete learners, publish courses, enrol people, issue publicly verified credentials, or create commercial transactions. See the [Learning Centre operating model](docs/learning-centre-of-excellence-operating-model.md), [cross-provider curriculum pathways](docs/cross-provider-curriculum-pathways.md), [cross-provider learning evidence operating model](docs/cross-provider-learning-evidence-operating-model.md), and [curated learning and private certification operating model](docs/curated-learning-and-private-certification-operating-model.md).

### AI-assisted operating layer

| Capability | What it does | What it intentionally does not do |
|---|---|---|
| **Institution Copilot** | Turns an owner/admin operating goal into a role-aware protected handoff, including the School Builder; it gives students only scoped learning-support guidance. | Perform a high-impact action from chat, bypass permissions, complete assessed work, grade, or decide learner outcomes. |
...
| **Course Studio** | Generates an editable internal programme, module, milestone, plain-language lesson guide, non-graded knowledge check, practice/project/revision material, tutor-scope draft, and transparent learning-experience profile using optional server-resolved curated references. | Claim curriculum approval/accreditation, calculate a score, grade, pass/fail result, progress change, completion, credential, launch a public course, or create learner outcomes without review. |
...
| **One-Prompt Institution Builder** | Makes “What do you want to build?” the primary owner experience. One natural-language prompt or template prepares an editable private institution plan with coherent module and lesson starters, practical projects, assessment-integrity guidance, learner experience, unpublished website/admissions/pricing readiness, a private original-brand/logo concept, private offer/growth foundation, quality checks, and protected lifecycle handoffs—including an Advertising review handoff. Refinements create a new private version rather than overwriting the current record. Its typed transport rejects document responses before they reach the JSON parser and gives the owner a safe recovery state when a secure data response is unavailable. | Create or license a brand asset, publish an institution, create people, admit or enrol learners, set fees, create a price/lead/campaign/testimonial/referral, spend, collect payments, send messages, grade, complete learners, issue credentials, or change provider/domain settings. A transport failure does not create, apply, publish, or audit a blueprint. |
| **Knowledge-to-Business and File-to-School Engine** | Lets an owner/admin paste approved text or confirmed-upload a bounded UTF-8 TXT, Markdown, or CSV source. Original supported file bytes remain in tenant-scoped object storage; the library preserves immutable source-revision snapshots and visibly labels source-based, AI-expanded, and AI-suggested planning content. It produces tenant-private expertise, curriculum, practical-project, offer, website-readiness, and quality-review analysis. Owners can inspect/revise/confirmed-delete stored sources and lineage, then separately confirm a Builder handoff or open existing protected Learning Centre, Website, Advertising, and supervised-tutor workspaces. | Parse PDFs, DOCX, PPTX, images, websites, video, or audio; silently add external research; verify source facts; create programme records, lessons, a website, price, campaign, lead, message, admission, enrolment, payment, assessment, learner outcome, completion, credential, portfolio, or scheduled work. It does not infer individual ability, readiness, risk, eligibility, or career prospects. |
| **AI School Operator** | Gives owners and administrators an on-demand, explainable view of tenant-scoped operating readiness: onboarding, programmes and active enrolments, pending admissions, outstanding invoices, email and automation failures, reviewed learning-evidence workload, private-record readiness, and website readiness. It can retain an owner-approved private operating profile, save review-display preferences, make owner authority explicit, and route a recommendation into its existing protected workspace. | Predict learner risk or demand, label individuals, change price, approve admissions, grade, alter progress, complete, enrol, certify, publish, create campaigns, spend, send messages, collect payments, retry external actions, start background work, or change providers, domains, or DNS. |
...
The AI boundary is a product advantage: models produce strict structured plans and drafts; deterministic server-side aggregation supplies School Operator signals; server-side code validates all inputs, owns the action catalog, and executes only allowlisted internal work. The typed client transport rejects a non-JSON response before it becomes a raw parsing error, and the Institution Builder fails closed with an explicit recovery state rather than applying a blueprint. The Knowledge-to-Business and File-to-School Engine accepts a deliberately bounded owner-authorised text source, independently validates supported UTF-8 uploads, keeps raw text and file bytes out of audit metadata, preserves private revision lineage, and requires separate confirmation before any Builder record is prepared. The system keeps human review where consequences become financial, public, identity-related, academic, or irreversible. See the [Knowledge-to-Business Engine contract](docs/knowledge-to-business-engine-contract.md), [File-to-School Engine contract](docs/file-to-school-engine-contract.md), [AI School Operator operating model](docs/ai-school-operator-operating-model.md), [builder-first phased institution contract](docs/builder-first-phased-institution-contract.md), [supervised AI operating model](docs/supervised-ai-agents-operating-model-2026-08-22.md), and [single-prompt launcher contract](docs/single-prompt-online-school-launcher-contract.md).
...
| Latest product additions | Tenant-private Knowledge-to-Business Engine with approved text sources, structured expertise/curriculum/project/offer/website-readiness analysis, source inspection/deletion, and confirmed Builder/protected-workspace handoffs; approval-first AI School Operator workflow preferences and visible owner-authority policy; learner-owned Student Learning Copilot next-step guidance; Institution Builder non-JSON response recovery and private brand/offer/growth planning; expanded review-only Course Studio material types; Automation Desk; Quick Start; Single-Prompt Online School Launcher; and the 19-Prompt Taster Library. |
| Focused validation | The Knowledge-to-Business release passed 41 focused tests across its source-analysis, protected-route, UI, Institution Builder, Course Studio, School Operator, and learner-support regressions, alongside fresh TypeScript validation and a production build. Migration `0062_unknown_radioactive_man` was reviewed and applied to add tenant-scoped private knowledge-source and analysis records. |
| Broader regression record | The latest full-suite record is **399/400 passing tests across 109/110 files**. The sole failure is the unchanged external sender-domain authorisation integration check for the currently unverified `resend.dev` sender. |
...
| AI education-business-engine delivery contract | [Approval-first delivery contract](docs/ai-education-business-engine-delivery-contract.md) |
| Knowledge-to-Business source and planning contract | [Knowledge-to-Business Engine](docs/knowledge-to-business-engine-contract.md) |
| File-to-School source library, provenance, and review contract | [File-to-School Engine](docs/file-to-school-engine-contract.md) |
