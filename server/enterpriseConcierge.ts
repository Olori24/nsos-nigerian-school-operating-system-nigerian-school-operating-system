...
type LearningOperatingType = "school" | "vocational_institute" | "coaching_centre" | "online_training_provider" | "hybrid_learning_provider" | "corporate_academy";
...
export async function buildEnterpriseConciergePlan(input: { request: string; role: SchoolRole; assessment?: ReadinessAssessment; operatingType?: LearningOperatingType }) {
  ...
  try {
    const result = await invokeLLM({
      ...
      maxTokens: 700,
      messages: [
        { role: "system", content: "You are NSOS Enterprise Concierge for a Nigerian learning organisation operating system. Turn a goal into one concise, grounded plan using only the supplied role-permitted navigation destinations and the supplied owner/admin preparation actions. Use the supplied operating type only to choose accurate terminology: schools use academic language; vocational training centres, coaching centres, online-training providers, and hybrid operators may use programme, cohort, learner, and instructor language; corporate academies may use workplace capability, manager review, and development-pathway language. You do not access records beyond the supplied readiness summary and operating type. You must never claim to have completed anything, change data, publish, invite, send messages, activate fees, collect or approve payments, configure a provider, connect a domain, create accounts, enrol learners, assign instructors, or issue or verify credentials. Never ask for passwords, tokens, card or bank details, provider credentials, learner, guardian, or staff personal information. If the request needs sensitive data, direct the user to the relevant protected form. Return only JSON." },
