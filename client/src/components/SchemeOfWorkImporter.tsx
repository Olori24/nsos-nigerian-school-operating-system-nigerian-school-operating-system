...
import { AlertTriangle, CheckCircle2, FileSpreadsheet, LockKeyhole, MessageSquareText, Replace, Upload, X } from "lucide-react";
import { ExpiredSchemeRecommendationCsvExport, ExpiredSchemeRecommendationReport, LeaderSchemeRevisionPriorityControls } from "@/components/SchemeRevisionPriorityControls";
...
function parseCsv(text: string) {
  ...
  for (let index = 0; index < source.length; index += 1) {
    ...
    if (character === '"') {
      if (quoted && source[index + 1] === '"') { field += '"'; index += 1; continue; }
      if (!quoted && field.length === 0) { quoted = true; continue; }
...
async function parseSchemeFile(file: File) {
  if (file.size > maxBytes) throw new Error("Upload a CSV scheme file no larger than 2 MB.");
  if (!/\.csv$/i.test(file.name)) throw new Error("Excel (.xlsx) imports are temporarily unavailable. Export the approved scheme as CSV and upload that file instead.");
  const table = parseCsv(await file.text());
  const headers = table.shift()?.map(cleanHeader) ?? [];
  if (!headers.length) throw new Error("The CSV needs a header row.");
  const sourceRows = table.filter(source => source.some(value => value.trim().length > 0));
  if (!sourceRows.length) throw new Error("The selected CSV does not contain any rows.");
  const rows = sourceRows.map((source, index) => {
    const values = Object.fromEntries(headers.map((header, column) => [header, (source[column] ?? "").trim()]));
...
export function SchemeOfWorkImporter({ schoolId, academic, staff, canConfigure, onDone }: { schoolId: number; academic: any; staff: any[]; canConfigure: boolean; onDone: () => void }) {
  ...
  const hasClasses = Boolean(academic?.classes?.length);
  const hasTerms = Boolean(academic?.terms?.length);
  const hasLinkedSubject = Boolean(subjectOptions.length);
  const activeTeachers = useMemo(() => staff.filter(item => item.employmentStatus === "active"), [staff]);
  const hasActiveTeacher = Boolean(activeTeachers.length);
  const importReady = Boolean(classId && subjectId && termId && selectedClassSubject?.teacherId);
  const readinessSteps = [
    { label: "Create at least one class", complete: hasClasses, note: "Use the Class setup tab in Academics." },
    { label: "Create the active academic term", complete: hasTerms, note: "Use the Term setup tab in Academics." },
    { label: "Apply an approved subject curriculum to the class", complete: !hasClasses || !classId ? Boolean(academic?.curriculumProfile) : hasLinkedSubject, note: "Use Nigerian curriculum & subject setup after creating the class." },
    { label: "Make at least one staff teacher active", complete: hasActiveTeacher, note: "Add or activate a teacher in Staff before assignment." },
    { label: "Assign an active reviewing teacher", complete: Boolean(selectedClassSubject?.teacherId), note: "Choose the teacher after selecting the class and subject below." },
  ];
  ...
  const submit = async () => {
    ...
    try { importScheme.mutate({ schoolId, classId: Number(classId), subjectId: Number(subjectId), termId: Number(termId), fileName: file.name, mimeType: "text/csv", base64: await asBase64(file), rows, replaceExisting }); }
    catch (error) { toast.error(error instanceof Error ? error.message : "The scheme file could not be prepared for import."); }
  };
  return <section className="rounded-[1.2rem] border border-[#d8e5da] bg-white p-5 shadow-[0_10px_32px_rgba(16,45,35,0.035)] sm:p-6"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div className="flex gap-3"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-[#eaf2ec] text-[#0f5c4f]"><FileSpreadsheet className="h-5 w-5" /></span><div><p className="text-sm font-semibold text-[#294238]">Approved scheme-of-work import</p><p className="mt-1 max-w-2xl text-xs leading-5 text-[#64766b]">Upload a school-approved CSV file, review its weeks and topics locally, then map it to an existing class, subject, and term. Importing never publishes lessons or alters results. Excel (.xlsx) imports are temporarily unavailable while NSOS uses this safer reviewed parser path.</p></div></div></div>
    {!canConfigure ? <div className="mt-5 flex gap-3 rounded-xl border border-[#e8e0c8] bg-[#fffaf0] p-4 text-xs leading-5 text-[#735d2f]"><LockKeyhole className="mt-0.5 h-4 w-4 shrink-0" />Only the school owner or an administrator can import or replace an approved scheme of work.</div> : <div className="mt-5 grid gap-4"><div role="status" className="rounded-xl border border-[#dce7de] bg-[#f8fcf8] p-4"><p className="text-xs font-bold text-[#315046]">Pre-import readiness</p><p className="mt-1 text-[11px] leading-5 text-[#607168]">Complete these school setup steps in Academics before uploading a scheme. Empty selectors are now explained rather than treated as an import error.</p><div className="mt-3 grid gap-2 sm:grid-cols-2">{readinessSteps.map(step => <div key={step.label} className={`flex gap-2 rounded-lg border px-3 py-2 text-[11px] leading-4 ${step.complete ? "border-[#cfe2d3] bg-white text-[#326149]" : "border-[#ead9bd] bg-[#fffaf0] text-[#755d2e]"}`}>{step.complete ? <CheckCircle2 className="mt-0.5 h-3.5 w-3.5 shrink-0" /> : <AlertTriangle className="mt-0.5 h-3.5 w-3.5 shrink-0" />}<span><strong>{step.complete ? "Ready: " : "Required: "}</strong>{step.label}<br /><span className="font-medium opacity-80">{step.note}</span></span></div>)}</div></div><div className="grid gap-3 md:grid-cols-3"><label className="grid gap-1.5 text-xs font-semibold text-[#43534c]"><span>Class</span><select disabled={!hasClasses} className={inputClass} value={classId} onChange={event => { setClassId(event.target.value); setSubjectId(""); }}><option value="">{hasClasses ? "Select class" : "Create a class first"}</option>{academic?.classes?.map((item: any) => <option value={item.id} key={item.id}>{item.name}</option>)}</select></label><label className="grid gap-1.5 text-xs font-semibold text-[#43534c]"><span>Approved subject</span><select disabled={!classId || !hasLinkedSubject} className={inputClass} value={subjectId} onChange={event => setSubjectId(event.target.value)}><option value="">{!classId ? "Select a class first" : hasLinkedSubject ? "Select subject" : "Apply curriculum to this class first"}</option>{subjectOptions.map((item: any) => <option value={item.id} key={item.id}>{item.name}</option>)}</select></label><label className="grid gap-1.5 text-xs font-semibold text-[#43534c]"><span>Term</span><select disabled={!hasTerms} className={inputClass} value={termId} onChange={event => setTermId(event.target.value)}><option value="">{hasTerms ? "Select term" : "Create a term first"}</option>{academic?.terms?.map((item: any) => <option value={item.id} key={item.id}>{item.name}</option>)}</select></label></div>{classId && subjectId && <div className="rounded-xl border border-[#dce7de] bg-[#f8fcf8] p-4"><p className="text-xs font-semibold text-[#315046]">Assigned reviewing teacher</p>{selectedClassSubject?.teacherId ? <p className="mt-1 text-xs leading-5 text-[#607168]">A teacher is assigned to this class-subject. Every imported week will remain pending until that teacher approves it.</p> : <><p className="mt-1 text-xs leading-5 text-[#735d2f]">Assign an active staff teacher before this scheme can be imported for review.</p>{!hasActiveTeacher && <p className="mt-2 rounded-lg border border-dashed border-[#e3cd9d] bg-white px-3 py-2 text-[11px] leading-5 text-[#755d2e]">No active teacher is available yet. Add or activate a staff teacher in the Staff workspace, then return here to assign that reviewer.</p>}<div className="mt-3 flex flex-col gap-2 sm:flex-row"><select disabled={!hasActiveTeacher} className={inputClass} value={assignedTeacherId} onChange={event => setAssignedTeacherId(event.target.value)}><option value="">{hasActiveTeacher ? "Select active teacher" : "Add an active staff teacher first"}</option>{activeTeachers.map(item => <option key={item.id} value={item.id}>{item.firstName} {item.lastName}</option>)}</select><button disabled={!assignedTeacherId || assignTeacher.isPending} onClick={() => assignTeacher.mutate({ schoolId, classId: Number(classId), subjectId: Number(subjectId), teacherId: Number(assignedTeacherId) })} className="rounded-xl border border-[#cde0d0] bg-white px-4 py-2 text-xs font-bold text-[#176145] disabled:opacity-50">{assignTeacher.isPending ? "Assigning…" : "Assign reviewer"}</button></div></>}</div>}<div className={`rounded-xl border border-dashed p-4 ${importReady ? "border-[#cbdccc] bg-[#f9fcf9]" : "border-[#ead9bd] bg-[#fffaf0]"}`}><label className={`flex flex-col items-center justify-center gap-2 text-center ${importReady ? "cursor-pointer" : "cursor-not-allowed"}`}><Upload className={`h-5 w-5 ${importReady ? "text-[#0f5c4f]" : "text-[#a17b32]"}`} /><span className="text-sm font-bold text-[#294238]">Choose approved CSV file</span><span className="text-xs text-[#687970]">Required columns: <strong>Week</strong> and <strong>Topic</strong>. Optional: Objectives, Resources. Maximum 2 MB. Excel (.xlsx) is temporarily unavailable.</span>{!importReady && <span className="text-[11px] font-semibold text-[#755d2e]">The upload unlocks after the required setup steps above are complete.</span>}<input disabled={parsing || !importReady} onChange={onFileChange} accept=".csv,text/csv" type="file" className="sr-only" /></label></div>{file && <div className="rounded-xl border border-[#d9e7db] bg-[#f8fcf8] p-4"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-semibold text-[#2f493d]">Ready for review: {file.name}</p><p className="mt-1 text-[11px] text-[#64766b]">{rows.length} unique week{rows.length === 1 ? "" : "s"} parsed locally. The original approved document will be retained securely after import.</p></div><button onClick={() => { setFile(null); setRows([]); }} className="rounded-lg p-1.5 text-[#6e7d75] hover:bg-white" aria-label="Remove selected scheme file"><X className="h-4 w-4" /></button></div><div className="mt-3 max-h-48 overflow-y-auto rounded-lg border border-[#e5ece6] bg-white"><table className="w-full text-left text-xs"><thead className="sticky top-0 bg-[#f4f8f4] text-[10px] uppercase tracking-[0.1em] text-[#738177]"><tr><th className="px-3 py-2">Week</th><th className="px-3 py-2">Topic</th><th className="px-3 py-2">Objectives</th></tr></thead><tbody>{rows.map(row => <tr key={row.weekNo} className="border-t border-[#edf1ed]"><td className="px-3 py-2 font-bold text-[#285143]">{row.weekNo}</td><td className="px-3 py-2 text-[#31483d]">{row.topic}</td><td className="px-3 py-2 text-[#6a7971]">{row.objectives || "—"}</td></tr>)}</tbody></table></div></div>}{currentTargetAlreadyImported && <label className="flex gap-2 rounded-xl border border-[#ead4b2] bg-[#fffaf0] p-3 text-xs leading-5 text-[#735824]"><input checked={replaceExisting} onChange={event => setReplaceExisting(event.target.checked)} type="checkbox" className="mt-0.5 h-4 w-4 accent-[#8a5a12]" /><span><strong>Replace the current approved scheme for this class, subject, and term.</strong> Existing imported curriculum milestones for this same mapping will be replaced; manual lesson plans and results are not changed.</span></label>}<button disabled={!file || !rows.length || !importReady || importScheme.isPending || (currentTargetAlreadyImported && !replaceExisting)} onClick={submit} className="inline-flex w-fit items-center gap-2 rounded-xl bg-[#0f5c4f] px-4 py-2.5 text-sm font-bold text-white disabled:cursor-not-allowed disabled:opacity-50"><Replace className="h-4 w-4" />{importScheme.isPending ? "Importing for teacher review…" : "Import for teacher review"}</button></div>}
    ...
    {canConfigure && <div className="mt-6 grid gap-6"><LeaderSchemeRevisionPriorityControls schoolId={schoolId} onDone={onDone} /><ExpiredSchemeRecommendationReport schoolId={schoolId} /><ExpiredSchemeRecommendationCsvExport schoolId={schoolId} /></div>}
  </section>;
}
