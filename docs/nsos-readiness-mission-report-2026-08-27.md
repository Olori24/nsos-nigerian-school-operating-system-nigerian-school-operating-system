...
| Resend sender health | **DNS remediation verified; provider-status reconciliation remains** | Resend identified a missing CNAME. The owner-approved DomainKing support correction now makes `send.nsos.top` resolve publicly to the exact required target, `send.forge.rmta.net.` Authenticated Resend list/detail checks nevertheless still returned `partially_failed`, with sending enabled and receiving disabled. One owner-confirmed factual reply was sent in the existing support thread asking only for status reconciliation or a non-secret condition. | Do not send email, enable inbound receiving, add mailboxes, alter recipients, or make another DNS change unless Resend supplies a new non-secret instruction. |
