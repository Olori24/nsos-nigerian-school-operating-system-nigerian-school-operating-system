  ...
  affiliates: router({
    partners: router({
      ...
      create: platformOwnerProcedure
        ...
        .mutation(({ ctx, input }) =>
          db.createAffiliatePartner({ ...input, createdBy: ctx.user.id })
        ),
    ...
    conversions: router({
      list: platformOwnerProcedure
        .input(
          z
            .object({ partnerId: z.number().int().positive().optional() })
            .optional()
        )
        .query(({ input }) => db.listAffiliateConversions(input?.partnerId)),
      ...
      approve: platformOwnerProcedure
        ...
        .mutation(({ ctx, input }) =>
          db.approveAffiliateConversion(input.id, ctx.user.id)
        ),
    }),
    payouts: router({
      ...
      create: platformOwnerProcedure
        ...
        .mutation(({ ctx, input }) =>
          db.createAffiliatePayout({ ...input, createdBy: ctx.user.id })
        ),
      updateStatus: platformOwnerProcedure
        ...
        .mutation(({ ctx, input }) =>
          db.updateAffiliatePayoutStatus({ ...input, approvedBy: ctx.user.id })
        ),
    }),
  }),
        ...
        schoolInput.extend({
          ...
          websiteContent: z
            .object({
              ...
              principalTitle: z.string().trim().max(160).optional(),
              principalMessage: z.string().trim().max(5000).optional(),
              programmes: z
    ...
    ask: onboardingAdminProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (!rate.allowed)
          throw new TRPCError({
            code: "TOO_MANY_REQUESTS",
            message: `Ask My School is taking a short break. Try again in about ${rate.retryAfterSeconds} seconds.`,
          });
        const result = await db.answerSchoolOperatorQuestion({
          schoolId: input.schoolId,
        ...
        await db.recordSecurityAuditEvent({
          schoolId: input.schoolId,
          actorUserId: ctx.user.id,
          eventType: "school_operator_question_answered",
    ...
    enrol: managementProcedure("students.write")
      ...
      .mutation(async ({ ctx, input }) => {
        const { admissionLetter, ...enrollment } =
          await db.enrolApplication(input);
