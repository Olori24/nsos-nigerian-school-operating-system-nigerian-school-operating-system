...
export type CourseStudioMaterialType = "facilitator_guide" | "lesson_guide" | "practice_activity" | "knowledge_check" | "project_brief" | "discussion_prompt" | "reflection_prompt" | "revision_sheet" | "resource_checklist";
...
const materialTypes = new Set<CourseStudioMaterialType>(["facilitator_guide", "lesson_guide", "practice_activity", "knowledge_check", "project_brief", "discussion_prompt", "reflection_prompt", "revision_sheet", "resource_checklist"]);
...
function fallbackDraft(input: CourseStudioRequest): CourseStudioDraft {
  ...
  return {
    ...
    materials: [
      ...
      { title: "Plain-language lesson guide", materialType: "lesson_guide", modulePosition: 1, content: "Explain one approved core idea in plain language, then use one organisation-reviewed example and ask learners to restate the idea in their own words. This is instructional guidance, not a score, examination, or completion decision." },
      ...
      { title: "Low-stakes knowledge check", materialType: "knowledge_check", modulePosition: 2, content: "Invite learners to answer two or three reflection questions about the approved topic, then review the explanation with a supervising instructor. Do not calculate a score, grade, pass/fail outcome, progress change, completion, or credential from this activity." },
      ...
      { title: "Revision sheet", materialType: "revision_sheet", modulePosition: 3, content: "Summarise the approved concepts, one practical example, common questions, and a learner-owned next practice step. A human instructor remains responsible for deciding any assessed work, score, result, completion, or credential." },
    ],
