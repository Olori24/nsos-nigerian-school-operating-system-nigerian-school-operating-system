...
export function StudentAiTutorHub({ schoolId }: { schoolId: number }) {
  ...
  const [response, setResponse] = useState<{ answer: string; studySteps: string[]; needsTeacherSupport: boolean; tutorName: string; interactionKey: string; adaptationEnabled: boolean; teachingStyle: string } | null>(null);
  ...
  const teachingPreference = trpc.nsos.aiTutors.setTeachingPreference.useMutation({
    onSuccess: async result => { await hub.refetch(); toast.success(result.adaptationEnabled ? "Feedback-informed explanation format is on." : "Feedback-informed explanation format is off. This tutor will use its balanced format."); },
    onError: error => toast.error(error.message),
  });
  ...
  return (
    <div className="grid gap-6">
      ...
      {!hub.data.studentLinked ? (
        ...
        <>
          {hub.data.learningRecommendation && <section className="rounded-[1.2rem] border border-[#cfe7d8] bg-[#f2fbf5] p-4"><div className="flex gap-3"><span className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-white text-[#176145]"><BookOpenCheck className="h-4 w-4" /></span><div><p className="text-xs font-extrabold text-[#29483b]">Your next safe learning step</p><p className="mt-1 text-sm font-semibold text-[#215e43]">{hub.data.learningRecommendation.title}</p><p className="mt-1 text-xs leading-5 text-[#5e766a]">{hub.data.learningRecommendation.detail}</p><p className="mt-2 text-[10px] font-semibold leading-4 text-[#5e766a]">This suggestion does not change your programme, progress, assessment, result, completion, or certificate. Follow the school’s approved learning plan and ask an instructor when unsure.</p></div></div></section>}
          <section className="rounded-[1.2rem] border border-[#e0e5df] bg-white p-5 shadow-[0_10px_32px_rgba(16,45,35,.035)] sm:p-6">
            <form onSubmit={submit} className="grid gap-4">
              ...
              {selected && <div className="rounded-xl bg-[#f5faf6] p-3 text-xs leading-5 text-[#65746c]"><p className="font-semibold text-[#29483b]">Approved scope</p><p className="mt-1">{selected.curriculumScope}</p><div className="mt-3 flex flex-wrap items-center justify-between gap-3 border-t border-[#dce8df] pt-3"><div><p className="font-semibold text-[#29483b]">Feedback-informed explanation format</p><p className="mt-0.5 text-[11px]">{selected.adaptationEnabled ? `On · current format: ${teachingStyleLabel(selected.teachingStyle)}` : "Off · balanced explanation format"}. This uses rating categories only, never your written feedback note.</p></div><button type="button" disabled={teachingPreference.isPending} onClick={() => teachingPreference.mutate({ schoolId, tutorId: selected.id, adaptationEnabled: !selected.adaptationEnabled })} className={`rounded-lg px-3 py-2 text-xs font-bold disabled:opacity-60 ${selected.adaptationEnabled ? "bg-[#e2f1e8] text-[#176145]" : "border border-[#cbdccf] bg-white text-[#416052]"}`}>{selected.adaptationEnabled ? "Turn off" : "Turn on"}</button></div></div>}
...
function teachingStyleLabel(style: string) {
  if (style === "step_by_step") return "step-by-step";
  if (style === "worked_examples") return "worked example";
  if (style === "concise_review") return "concise review";
  return "balanced";
}
