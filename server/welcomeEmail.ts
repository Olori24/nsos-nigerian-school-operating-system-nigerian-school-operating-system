...
function safeVerificationUrl(value: string | undefined, origin: string) {
  ...
  try {
    ...
    if (
      url.origin !== origin ||
      url.pathname !== "/api/auth/email/verify-address" ||
      !token ||
      !/^[A-Za-z0-9_-]{43}$/.test(token) ||
      Array.from(url.searchParams.keys()).some(key => key !== "token")
    )
      return undefined;
    return url.toString();
  } catch {
...
export function buildWelcomeEmailContent(input: {
  ...
  const safeVerificationLink = safeVerificationUrl(
  ...
  const escapedVerificationLink = safeVerificationLink
    ? escapeHtml(safeVerificationLink)
    : undefined;
  ...
  const quickLinksHtml = quickLinks
    ...
    .join("");
  const verificationText = escapedVerificationLink
    ? `Verify your email address: ${escapedVerificationLink}\n\n`
    : "";
  const verificationHtml = escapedVerificationLink
    ? `<div style="margin:28px 0 0;border-top:1px solid ${BRAND.border};padding-top:24px;"><h2 style="margin:0 0 10px;color:${BRAND.ink};font-size:18px;line-height:1.3;">Verify your email</h2><p style="margin:0;color:${BRAND.muted};font-size:13px;line-height:1.6;">Confirm this address to keep your account contact details up to date.</p><table role="presentation" cellpadding="0" cellspacing="0" border="0" style="margin:18px 0 0;"><tr><td style="border-radius:10px;background:${BRAND.pine};"><a href="${escapedVerificationLink}" style="display:inline-block;padding:12px 18px;border:1px solid ${BRAND.pine};border-radius:10px;color:${BRAND.white};font-size:14px;font-weight:700;text-decoration:none;">Verify email address</a></td></tr></table><p style="margin:14px 0 0;color:${BRAND.muted};font-size:12px;line-height:1.5;word-break:break-all;">Or open: ${escapedVerificationLink}</p></div>`
    : "";
  const text = `Hi ${firstName},\n\nYour NSOS account is ready.\n\n${verificationText}Sign in here: ${signInUrl}\n\nGetting started\n${quickLinksText}\n\nIf you did not create this account, you can safely ignore this email.`;
...
<p style="margin:16px 0 0;color:${BRAND.muted};font-size:16px;line-height:1.65;">Your NSOS account is ready. Use the secure button below to open your workspace.</p>${verificationHtml}<table role="presentation" cellpadding="0" cellspacing="0" border="0" style="margin:28px 0 0;">
...
export async function dispatchWelcomeEmailForNewAccount(input: {
  ...
  try {
    ...
    const verificationToken = await db.createEmailVerificationToken({
      userId: input.userId,
      redirectOrigin: origin,
    });
    const verificationUrl = new URL("/api/auth/email/verify-address", origin);
