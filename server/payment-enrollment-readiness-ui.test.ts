...
const readiness = readFileSync(
  resolve(root, "client/src/components/PaymentEnrollmentReadiness.tsx"),
  "utf8"
);
...
describe("payment integration and enrollment readiness interface", () => {
  it("mounts the owner/admin evidence panel in the active Builder-first overview", () => {
    ...
    expect(home).toContain(
      "<PaymentEnrollmentReadiness schoolId={schoolId} onNavigate={onNavigate} />"
    );
  ...
  it("uses only existing protected evidence queries and provides finance and admissions handoffs", () => {
    expect(readiness).toContain(
      "trpc.nsos.providers.list.useQuery({ schoolId })"
    );
    expect(readiness).toContain(
      "trpc.nsos.cashAssurance.list.useQuery({ schoolId })"
    );
    expect(readiness).toContain(
      "trpc.nsos.admissions.list.useQuery({ schoolId, limit: 100 })"
    );
  ...
  it("states the provider-evidence and no-auto-enrollment boundaries explicitly", () => {
    ...
    expect(readiness).toContain(
      "does not create a learner, alter an application, assign a class, send an admission letter, or confirm enrollment from this panel"
    );
    expect(readiness).toContain(
      "cannot save merchant credentials, run a provider test, collect or record a payment, approve payment evidence, accept an applicant, enroll a learner, issue a receipt, or send an admission letter"
    );
  });
});
