...
export const nsosRouter = router({
  ...
  website: router({
    ...
    save: websiteAdminProcedure
      .input(schoolInput.extend({ headline: z.string().max(255).optional(), introduction: z.string().max(5000).optional(), primaryColor: z.string().regex(/^#[0-9a-fA-F]{6}$/).optional(), contactEmail: z.string().email().optional(), contactPhone: z.string().max(48).optional(), campusLocation: z.string().max(255).optional(), customDomain: customDomainInput, admissionsEnabled: z.boolean().optional(), logoMediaId: z.number().int().positive().nullable().optional(), heroMediaId: z.number().int().positive().nullable().optional(), visualTheme: z.enum(["modern", "academic", "community"]).optional(), websiteContent: z.object({ about: z.string().trim().max(5000).optional(), principalName: z.string().trim().max(160).optional(), principalTitle: z.string().trim().max(160).optional(), principalMessage: z.string().trim().max(5000).optional(), programmes: z.array(z.string().trim().min(2).max(180)).max(24).optional(), faqs: z.array(z.object({ question: z.string().trim().min(3).max(240), answer: z.string().trim().min(3).max(1200) })).max(24).optional(), socialLinks: z.array(z.object({ label: z.string().trim().min(2).max(40), url: z.string().url().max(2048) })).max(8).optional() }).nullable().optional(), published: z.boolean().optional() }))
      .mutation(async ({ ctx, input }) => {
