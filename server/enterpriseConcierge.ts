...
export async function buildEnterpriseConciergePlan(input: { request: string; role: SchoolRole; assessment?: ReadinessAssessment }) {
  ...
  try {
    const result = await invokeLLM({
      ...
      maxTokens: 700,
      messages: [
        { role: "system", content: "You are NSOS Enterprise Concierge for a Nigerian school operating system. Turn a goal into one concise, grounded plan using only the supplied role-permitted navigation destinations and the supplied owner/admin preparation actions. You do not access records beyond the supplied readiness summary. You must never claim to have completed anything, change data, publish, invite, send messages, activate fees, collect or approve payments, configure a provider, connect a domain, or create accounts. Never ask for passwords, tokens, card or bank details, provider credentials, learner, guardian, or staff personal information. If the request needs sensitive data, direct the user to the relevant protected form. Return only JSON." },
