...
import { AlertCircle, ArrowDownUp, Bell, Check, Download, FileUp, FilterX, HandCoins, ShieldCheck, Sparkles } from "lucide-react";
...
export function FamilyCashAssurance({ schoolId, role, onDone }: { schoolId: number; role: "parent" | "student"; onDone: () => void }) {
  ...
  const notificationQuery = trpc.nsos.portal.paymentEvidenceNotifications.useQuery({ schoolId });
  ...
  const [historySort, setHistorySort] = useState<"newest" | "oldest" | "highest" | "lowest" | "status">("newest");
  ...
  const familyHistory = useMemo(() => {
    const payments = paymentHistory.map((item: any) => ({ id: `payment-${item.id}`, kind: "payment" as const, status: "confirmed", amount: Number(item.amount), date: item.paidOn, learner: item.student ? `${item.student.firstName} ${item.student.lastName}` : "Linked learner", receiptNo: item.receiptNo, invoiceNo: item.invoiceNo, method: item.method }));
    const evidence = evidenceRows.map((item: any) => {
      const relatedCase = query.data?.cases.find((caseItem: any) => caseItem.id === item.caseId);
      return { id: `evidence-${item.id}`, kind: "evidence" as const, status: item.status === "accepted" ? "approved" : item.status === "rejected" ? "rejected" : "pending", amount: Number(item.amountClaimed), date: item.createdAt, learner: relatedCase?.student ? `${relatedCase.student.firstName} ${relatedCase.student.lastName}` : "Linked learner", receiptNo: null, invoiceNo: null, method: item.source };
    });
    const fromTime = historyFrom ? new Date(`${historyFrom}T00:00:00`).getTime() : Number.NEGATIVE_INFINITY;
    const toTime = historyTo ? new Date(`${historyTo}T23:59:59`).getTime() : Number.POSITIVE_INFINITY;
    const rows = [...payments, ...evidence].filter(item => {
      const timestamp = new Date(item.date).getTime();
      return timestamp >= fromTime && timestamp <= toTime && (historyStatus === "all" || item.status === historyStatus);
    });
    return rows.sort((left, right) => {
      if (historySort === "oldest") return new Date(left.date).getTime() - new Date(right.date).getTime();
      if (historySort === "highest") return right.amount - left.amount;
      if (historySort === "lowest") return left.amount - right.amount;
      if (historySort === "status") return left.status.localeCompare(right.status) || new Date(right.date).getTime() - new Date(left.date).getTime();
      return new Date(right.date).getTime() - new Date(left.date).getTime();
    });
  }, [evidenceRows, historyFrom, historySort, historyStatus, historyTo, paymentHistory, query.data?.cases]);
  const unreadNotifications = (notificationQuery.data ?? []).filter((item: any) => !item.readAt);
  const acknowledgeNotification = trpc.nsos.portal.markPaymentEvidenceNotificationRead.useMutation({
    onSuccess: async () => { await utils.nsos.portal.paymentEvidenceNotifications.invalidate({ schoolId }); },
  ...
  return <section className="grid gap-5" aria-label="Payment promises and evidence">
    <div className="rounded-2xl border border-[#cce2d0] bg-[#f5fbf5] p-5"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div className="flex gap-3"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-[#0f5c4f] text-white"><HandCoins className="h-4 w-4" /></span><div><p className="text-sm font-semibold text-[#1c4033]">Payment promises & evidence</p><p className="mt-1 max-w-2xl text-xs leading-5 text-[#587267]">View payment commitments for {role === "parent" ? "your linked learners" : "your school account"} and send proof of payment directly to the finance team.</p></div></div><div className="flex items-center gap-2">{unreadNotifications.length > 0 && <span className="inline-flex items-center gap-1 rounded-full bg-[#b54536] px-2.5 py-1 text-[10px] font-bold text-white"><Bell className="h-3 w-3" />{unreadNotifications.length} new update{unreadNotifications.length === 1 ? "" : "s"}</span>}<Badge tone="good">Finance reviewed</Badge></div></div><p className="mt-4 flex items-center gap-2 text-[11px] text-[#567466]"><ShieldCheck className="h-3.5 w-3.5" />Submitting evidence does not record a payment or change your balance. The school must review it first.</p></div>

    {unreadNotifications.length > 0 && <section className="overflow-hidden rounded-2xl border border-[#f1d3cf] bg-white" aria-label="New payment evidence decisions"><div className="flex items-center justify-between gap-3 border-b border-[#f5e6e3] px-5 py-4"><div><p className="text-sm font-semibold text-[#6e3027]">New payment evidence updates</p><p className="mt-1 text-xs text-[#976057]">Finance has completed a review of your submitted evidence.</p></div><Bell className="h-4 w-4 text-[#b54536]" /></div><div>{unreadNotifications.map((notification: any) => { const approved = notification.decision === "accepted"; const learner = notification.student ? `${notification.student.firstName} ${notification.student.lastName}` : "your linked learner"; return <div key={notification.id} className="flex flex-col gap-3 border-b border-[#f8eeee] px-5 py-4 last:border-0 sm:flex-row sm:items-center sm:justify-between"><div><div className="flex flex-wrap items-center gap-2"><p className="text-sm font-semibold text-[#374c42]">{approved ? "Payment evidence approved" : "Payment evidence rejected"}</p><Badge tone={approved ? "good" : "danger"}>{approved ? "Approved" : "Rejected"}</Badge></div><p className="mt-1 text-xs leading-5 text-[#64746c]">{currency(Number(notification.amountClaimed))} submitted for {learner} on {displayDate(notification.createdAt)}. {approved ? "Finance has confirmed a matching recorded payment." : "Finance could not match this evidence to a recorded payment. Please review the document or contact the school finance team."}</p></div><button type="button" disabled={acknowledgeNotification.isPending} onClick={() => acknowledgeNotification.mutate({ schoolId, notificationId: notification.id })} className="inline-flex shrink-0 items-center justify-center gap-1.5 rounded-lg border border-[#d8e5da] bg-[#f8fcf8] px-3 py-2 text-xs font-semibold text-[#176145] disabled:cursor-not-allowed disabled:opacity-50"><Check className="h-3.5 w-3.5" />Mark read</button></div>; })}</div></section>}
    ...
    <section className="overflow-hidden rounded-2xl border border-[#e1e8e1] bg-white" aria-label="Family payment history">
      <div className="border-b border-[#edf0eb] px-5 py-4"><div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-sm font-semibold text-[#294239]">Payment history</p><p className="mt-1 text-xs text-[#758079]">Filter confirmed payments and payment-evidence updates from this portal account.</p></div><p className="text-xs font-semibold text-[#567466]">{familyHistory.length} record{familyHistory.length === 1 ? "" : "s"}</p></div><div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-5"><label className="grid gap-1"><span className="text-[10px] font-semibold uppercase tracking-[0.1em] text-[#718077]">From</span><input type="date" className={inputClass} value={historyFrom} onChange={event => setHistoryFrom(event.target.value)} /></label><label className="grid gap-1"><span className="text-[10px] font-semibold uppercase tracking-[0.1em] text-[#718077]">To</span><input type="date" className={inputClass} value={historyTo} onChange={event => setHistoryTo(event.target.value)} /></label><label className="grid gap-1"><span className="text-[10px] font-semibold uppercase tracking-[0.1em] text-[#718077]">Status</span><select className={inputClass} value={historyStatus} onChange={event => setHistoryStatus(event.target.value)}><option value="all">All records</option><option value="confirmed">Confirmed payments</option><option value="pending">Pending review</option><option value="approved">Approved evidence</option><option value="rejected">Rejected evidence</option></select></label><label className="grid gap-1"><span className="text-[10px] font-semibold uppercase tracking-[0.1em] text-[#718077]">Sort by</span><select className={inputClass} value={historySort} onChange={event => setHistorySort(event.target.value as typeof historySort)}><option value="newest">Newest date</option><option value="oldest">Oldest date</option><option value="highest">Highest amount</option><option value="lowest">Lowest amount</option><option value="status">Status</option></select></label><button type="button" onClick={() => { setHistoryFrom(""); setHistoryTo(""); setHistoryStatus("all"); setHistorySort("newest"); }} className="mt-auto inline-flex h-10 items-center justify-center gap-1.5 rounded-lg border border-[#dce7de] px-3 text-xs font-semibold text-[#36564a] hover:bg-[#f7fbf7]"><FilterX className="h-3.5 w-3.5" />Clear filters</button></div></div>
      {familyHistory.length ? <div>{familyHistory.map((item: any) => { const confirmed = item.status === "confirmed"; const outcome = confirmed ? { label: "Confirmed payment", tone: "good" as const } : evidenceOutcome(item.status === "approved" ? "accepted" : item.status === "rejected" ? "rejected" : "submitted"); return <div key={item.id} className="grid gap-3 border-b border-[#edf0eb] px-5 py-4 last:border-0 sm:grid-cols-[1fr_auto]"><div><div className="flex flex-wrap items-center gap-2"><p className="text-sm font-semibold text-[#30473b]">{currency(item.amount)} · {item.learner}</p><Badge tone={outcome.tone}>{outcome.label}</Badge></div><p className="mt-1 text-[11px] text-[#758079]">{displayDate(item.date)} · {confirmed ? `${String(item.method).replaceAll("_", " ")} · Receipt ${item.receiptNo}${item.invoiceNo ? ` · Invoice ${item.invoiceNo}` : ""}` : `${String(item.method).replaceAll("_", " ")} evidence`}</p></div><div className="flex items-start sm:justify-end"><span className="inline-flex items-center gap-1 text-[10px] font-semibold uppercase tracking-[0.1em] text-[#77857c]"><ArrowDownUp className="h-3 w-3" />{confirmed ? "Payment" : "Evidence"}</span></div></div>; })}</div> : <div className="px-5 py-6 text-sm text-[#758079]">No payment history matches the selected date, status, or sort filters.</div>}
    </section>
