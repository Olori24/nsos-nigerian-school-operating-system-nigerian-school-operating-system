...
const themeToggle = readFileSync(
  resolve(projectRoot, "client/src/components/BiodataThemeToggle.tsx"),
  "utf8"
);
...
describe("biodata dark-mode preference", () => {
  ...
  it("mounts a persistent switchable theme and covers preview, recovery, validation, and clear-form low-light states", () => {
    ...
    expect(appSource).toContain("z-50 sm:bottom-5 sm:right-5 sm:z-[70]");
    expect(themeToggle).toContain("h-11 w-11");
    expect(themeToggle).toContain("sr-only sm:not-sr-only");
  ...
  it("exposes a persistent accessible dark-mode control in owner/admin dashboard settings", () => {
    ...
    expect(homeSource).toContain(
      'document.getElementById("dashboard-appearance-settings")'
    );
