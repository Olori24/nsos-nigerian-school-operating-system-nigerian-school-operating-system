...
export async function answerSchoolOperatorQuestion(input: { schoolId: number; question: string }) : Promise<SchoolOperatorQuestionResult> {
  ...
  if (/(admission|applicant|application)/.test(normalized)) {
  ...
  if (/(fee|finance|revenue|money|outstanding|invoice)/.test(normalized)) {
  ...
  if (/(health|healthy|status|doing|operating state)/.test(normalized)) {
  ...
  if (/(setup|onboard|ready|launch)/.test(normalized)) {
  ...
  if (/(academic|assessment|score|result|learning|performance)/.test(normalized)) {
