...
- [x] Close the TinyURL redirect request as superseded: `nsos.top` and `www.nsos.top` are now live project domains, so no additional public short-link redirect was created or is needed.
...
- [x] Audit the current owner, student, School Builder, onboarding, Automation Desk, website, admissions, finance, learning, and copilot journeys for configuration complexity that can be replaced with safe AI-first defaults or clear protected handoffs.
- [x] Define a complete private institution-blueprint and publishing-readiness contract covering meaningful learning content, projects, assessment readiness, revenue readiness, admissions readiness, quality checks, owner approvals, versioning, and explicit unavailable external dependencies.
- [x] Make “What do you want to build?” the primary owner experience with a large natural-language School Builder entry, reusable institution templates, truthful staged preparation, and no technical configuration requirement for the initial private blueprint.
- [x] Extend protected institution-blueprint planning with coherent lesson, project, assessment, learner-experience, website, admissions, pricing, payments-readiness, notifications-readiness, analytics, and quality-readiness recommendations without fabricating people, transactions, outcomes, or external integrations.
- [x] Add explicit owner-confirmed progressive application and reversible private draft-version controls so users can selectively apply internal learning and website drafts while publication, enrolment, payments, communications, credentials, and provider changes remain separate approvals.
- [x] Refine the owner and student conversational assistants into role-scoped institution copilots that provide safe operating handoffs, learner-context explanation/practice support, and assessment-integrity safeguards without autonomous high-impact actions or cross-tenant exposure.
- [x] Add controlled lifecycle/revenue-readiness/quality-check visibility for onboarding, inactive learners, completion follow-up, pricing opportunities, and launch blockers without sending messages, changing fees, collecting payments, or issuing credentials automatically.
- [x] Add builder-first UX, template, AI-output, tenant-isolation, approval-boundary, versioning, quality-readiness, copilot, and mobile regressions; validate, document the phased rollout, checkpoint, publish, and synchronize GitHub safely.
- [x] Audit the existing AI, learning, admissions, finance, analytics, certification, automation, health, and multi-institution capabilities for reusable components and safe gaps in the autonomous school-operations vision.
...
- [x] Build the next approval-first knowledge-to-business engine release: tenant-private knowledge-source ingestion, structured expertise analysis, reviewable outcome-first curriculum/project/offer/website recommendations, protected handoffs, safety regressions, documentation, publication, and GitHub synchronization.
- [x] Build the next approval-first knowledge-to-business engine release: tenant-private knowledge-source ingestion, structured expertise analysis, reviewable outcome-first curriculum/project/offer/website recommendations, protected handoffs, safety regressions, documentation, publication, and GitHub synchronization.
...
- [x] Add deterministic CI validation gates for formatting, TypeScript, build, and non-provider-dependent test coverage; keep live sender-domain health as a separately reported external gate and do not change runtime, staging, provider, recovery, load, or email-delivery configuration. CI now uses locked install, production audit, lint, format, typecheck, disposable MySQL 8.4 migrations, deterministic tests, and build, with `RUN_LIVE_PROVIDER_TESTS=false`; local validation passed formatting, lint, typecheck, 127 files / 462 tests (one intentional live-provider skip), and production build.
- [x] Add and validate a privacy-safe NSOS liveness/readiness endpoint with no credentials, tenant data, provider state, or infrastructure topology; document that it supports deployment probes only and does not replace monitoring, backup, recovery, or load evidence. `GET /healthz` returns only the fixed service label, `ok` status, and UTC timestamp with no-store caching; TypeScript, focused regressions, build, and a local 200 response check passed.
...
- [x] Diagnose and repair the tester-facing “School website unavailable” state, using read-only publication and routing inspection before any owner-confirmed public-site change. Root cause was two separate platform-host guards that sent nsos.top/www.nsos.top into tenant school-domain resolution; both now share a tested product-host helper. After deployment, cache-busted browser checks confirmed nsos.top renders the NSOS entry and www redirects there, with no tenant publication changes.
...
- [ ] Provision the owner-approved isolated NSOS Staging application, empty database, storage namespace, deny-by-default provider stubs, disposable synthetic role labels, recovery/abort authority, and redacted evidence location; do not run recovery or load work in this setup step.
- [x] Resolve the staging database production-boundary ambiguity with a separately verifiable non-production database identity, or teardown the halted NSOS Staging environment; do not bypass, weaken, inspect, or query the rejected target. The owner selected teardown; the rejected target was never accessed and the staging guard remains unchanged.
- [x] Execute the owner-authorized teardown of only the halted NSOS Staging setup, preserve its sanitized boundary-halt evidence, and verify that no staging-specific runtime or temporary artifact remains. The task-local process, listener, socket, source, build, cache, configuration, stub, role-label, and temporary artifacts were removed; only redacted halt and teardown-verification evidence remains for the documented 30-day retention window.
- [ ] Arrange a user-approved separately identifiable non-production database for NSOS Staging, connect it only through secure project settings, and verify its staging identity and empty state before any recovery or load action.
- [ ] Maintain the owner-selected pause on new staging-provider, database, recovery, and load activity until a future independently verifiable non-production route is separately approved.
- [x] Replace the public client error-boundary stack-trace display with a privacy-safe recovery screen and regression coverage, without sending error data to a provider or changing user workflows. The recovery screen now exposes only an opaque local reference, safe retry/home actions, and no stack trace; TypeScript, focused tests, and the production build passed.
- [ ] Run and record a fresh full NSOS test-suite validation after the recent routing, authentication, observability, and client-recovery changes, without any staging provider, database, recovery, or load activity.
- [ ] Investigate the fresh full-suite external sender-domain failure read-only: Resend now reports `nsos.top` as `partially_failed`; do not change sender, provider, DNS, domain, email delivery, or test expectations without separate owner approval.
- [x] Prepare and, after explicit confirmation, submit a redacted Resend support inquiry for the `nsos.top` `partially_failed` inconsistency; do not change sender, DNS, mailbox, provider configuration, or send email. The public form returned a generic error, but the distinct authenticated in-account Contact us channel confirmed “Your message was sent” at High priority; no case reference or configuration change was shown.
- [ ] Await and record Resend’s support response or case reference for the `nsos.top` `partially_failed` inconsistency; do not change any email configuration or infer a cause before provider guidance.
- [x] Define an approval-gated NSOS branded operational email model covering support, security, notifications, admissions, billing, and sender/reply boundaries; do not create mailboxes, enable inbound mail, alter DNS, or change production sender behavior without separate owner approval. The documented model separates the existing transactional sender from proposed human-owned support and operational inboxes, with explicit activation evidence.
- [x] Perform the owner-selected non-mutating Resend sender re-verification check and research lower-cost branded mailbox options without creating accounts, mailboxes, aliases, DNS records, sender changes, or email delivery. Resend remained `partially_failed` after re-verification despite three listed TXT/MX records reporting verified; Zoho Mail free-plan eligibility and Migadu low-volume pricing were documented without signup or configuration.
- [x] Resolve the selected staging-database provider’s non-rendering sign-up path by using an accessible secure alternative; do not reuse or relax the rejected staging database boundary. The accessible Aiven path created only the separately labelled free-tier `nsos-staging-mysql` service in provider project `nsos-staging-db`; it is not connected to live NSOS.
- [x] Resolve the staging URI clipboard blocker by transferring the Aiven URI directly between protected provider and NSOS Staging secret interfaces only after explicit confirmation; never retain or expose it in chat, source, logs, or live NSOS settings. The staging-namespaced secret is present in NSOS Staging only; its value was not retained or used by the application or task.
- [ ] Remediate the staging read-only boundary failure by creating a distinct, staging-scoped least-privilege database principal with no global and no mutation-capable grants; do not reuse the current broad-privilege reference for any catalog, runtime, recovery, or load operation.
- [ ] Create the owner-approved `nsos_staging` database and dedicated least-privilege Aiven service user, securely replace the NSOS Staging URI with that user’s reference, and reverify the grant boundary before any staging workload.
- [ ] Apply the owner-approved `nsos_staging.*`-only grant to `nsos_staging_runtime`, securely replace the staging reference with that user’s URI, and remove the broad staging reference from use before any runtime, recovery, or load action.
- [ ] Resolve the failed scoped-grant verification through a provider-supported restricted-principal path; the attempted grant did not prove removal of global/out-of-scope privileges, so do not use `nsos_staging_runtime` or any broad staging reference for application, catalog, recovery, or load work.
- [ ] Execute the owner-approved revoke-and-regrant remediation for `nsos_staging_runtime`: remove inherited global/out-of-scope privileges, grant only `nsos_staging.*` permissions, then verify the effective boundary before replacing any staging runtime reference.
- [x] Dispose of or obtain a provider-supported independent verification path for the unusable Aiven staging database: after final revoke/regrant, the dedicated user still did not prove a target-only boundary, so do not use the current service or either staging credential for runtime, migration, recovery, or load work. The owner selected disposal and Aiven confirmed service termination.
- [x] Remove the now-stale `NSOS_STAGING_DATABASE_URL` value from the NSOS Staging secret interface without reading or reproducing it, then verify only that the secret is absent. The authenticated Application secrets list was visibly empty; no secret value was opened, copied, or retained.
- [x] Execute the owner-authorized removal of the stale NSOS Staging database secret through project settings, preserving no credential value and verifying only that the key is absent afterward. The empty secret list confirms completion while the deleted provider service prevents reuse.
- [x] Resolve the mobile NSOS Staging settings panel rendering failure before attempting the approved stale-secret removal; do not publish, add, or edit any project setting while the panel is blank. The Settings → Secrets panel recovered and showed the empty Application secrets list; no project setting was changed other than the confirmed stale-secret cleanup.
...
- [x] Execute the owner-approved fresh disposable MySQL 8.4 migration rehearsal using only the checksum-recorded staging repair artifact; require isolated-target preflight, sanitized full-chain/schema evidence, and confirmed teardown, with no live NSOS, existing staging, provider, domain, storage, payment, or learner-data access. The original artifact exposed an additional `0003` incompatibility and was safely contained; the separately reviewed expanded four-file artifact then passed the fresh MySQL 8.4.6 full chain, no-op rerun, schema checks, and teardown without non-disposable access.
...
- [x] Validate the end-to-end reference-academy readiness path using no real people, payments, certificates, messages, or provider-side changes; publish a factual PASS/WARNING/BLOCKED scorecard and checkpoint. Focused readiness checks passed 33/33; public paid launch remains blocked pending separate content, merchant, certificate, staging/recovery, and measured-capacity evidence.
...
- [x] Add editable, bounded, server-sanitized subject and body text to the confirmation-gated student-record PDF email-share workflow. Defaults are returned only from the authorized record, the UI enforces 180/3000-character bounds, server validation rejects subject line breaks, body HTML is escaped into a fixed wrapper, and neither raw copy nor recipient address enters audit metadata.
...
- [x] Inspect and complete a confirmation-gated Resend sender-domain verification handoff for `nsos.top`, including exact provider-issued DNS records and sender-readiness validation; owner approvals, public DNS evidence, Resend sending verification, and a no-send authorization test are documented.
- [x] Request and document a DomainKing-supported DNS-only management path for `nsos.top` because the available zone-creation flow requires an unsupplied IP address; do not invent an A record or alter unrelated domain settings. DomainKing Support ticket #738432 is answered; it requires either an application-provider IP or the registrar’s default IP.
- [x] Evaluate DomainKing’s requirement for a DNS-zone IP and identify a provider-approved, routing-safe target for `nsos.top`; Manus issued `104.18.26.246`, so DomainKing’s default IP was not used.
- [x] Obtain the exact Manus-issued A or CNAME target for the owner-selected `nsos.top` custom-domain path before creating a DomainKing DNS zone or adding any Resend record.
- [x] Apply one owner-approved DomainKing DNS set: root and `www` targets issued by Manus plus the exact three Resend sender-verification records; verify both website-domain and sender-domain readiness without a real email send.
- [x] Review and reconcile the DomainKing-generated `ftp`, `mail`, `pop`, `smtp`, `www`, root MX, and root SPF records created with the new zone; preserve them unchanged after owner review because the root and `www` A records already match the Manus target.
- [x] Correct the sender-authorization test’s domain extraction and Resend sending-only verification semantics so it validates standard `Display Name <address@domain>` syntax, verified provider records, and enabled sending capability without weakening the sender-domain gate.
...
- [x] Add a correlation-safe Express unexpected-error event and a stable request-ID response header, with no error body, credential, token, recipient, or query-string logging. TypeScript and the four-case observability regression pass.
- [x] Prepare an isolated NSOS staging handoff with synthetic-only data, provider stubs, recovery authority, and restricted test access; the handoff guided creation of a separate project and source-only baseline import. Do not run synthetic work until source-compatibility and remaining isolation-release gates are resolved.
- [ ] Collect the approved staging environment’s non-secret deployment, data/storage isolation, provider-stub, disposable-test-role, recovery-authority, and evidence-retention details before requesting final provisioning confirmation.
- [x] Assess the available managed-project path for a separate NSOS staging environment and verify that the owner can create an independent managed project before any data, provider, or workload setup.
- [x] Create one owner-approved empty `NSOS Staging` managed project with no custom domain, copied live data, live-provider integration, or workload execution. The created project has no configured website, connector, source, schedule, or deployment at creation; data/storage/secrets isolation must still be configured and verified before synthetic setup.
...
- [x] Obtain separate approval and a disposable MySQL 8.4 environment to execute the repaired full migration chain after journal/column preflight; do not connect to an existing or live schema. An initial rehearsal safely exposed the out-of-scope `0003` incompatibility and was destroyed; a newly reviewed four-file repair then passed its full retained migration chain on a fresh MySQL 8.4.6 target, with 65 journal rows, no-op rerun evidence, targeted repaired-object checks, and verified teardown. This is fresh-target compatibility evidence only.
- [x] Conduct the owner-approved source-only MySQL 8.4 migration compatibility review: scan the retained chain, create a separately checksummed repair artifact for 0003 and any further verified incompatibilities, and run source-only validation without any database, live/staging data, provider, domain, secret, or publication action. The new four-file artifact (`0002`, `0003`, `0005`, `0007`) has SHA-256 `4104e083f3262d34de00e310ed74d47f13b772b0cff2b568d2bf2dc6464cb7d1`, reverse-applied cleanly in its isolated worktree, left no conditional index clauses, and passed source-only locked install, TypeScript, and production-build validation with a deliberately unreachable database URL.
- [x] Execute the owner-approved fresh MySQL 8.4 rehearsal using only the four-file artifact with SHA-256 `4104e083f3262d34de00e310ed74d47f13b772b0cff2b568d2bf2dc6464cb7d1`; require fresh-target preflight, full-chain and no-op evidence, then mandatory target teardown regardless of result. The fresh MySQL 8.4.6 run applied all 65 migrations, passed a 65-row no-op rerun, verified targeted repaired objects and aggregate schema counts, then removed the target, bridge, runner credentials, patch copy, and temporary source workspace.
- [x] Remove visible Manus account/sign-in wording and controls from the NSOS public entry so Google and passwordless email are the only public authentication choices, while preserving existing Google/email security safeguards and underlying operational infrastructure. The legacy public button and unused Manus dialog component were removed; Google/email routes and their focused security regressions remain intact.
- [x] Refine the mobile first-visit NSOS interface alongside the sign-in simplification: improve hierarchy and spacing, prevent install/theme overlays from competing with authentication controls, and preserve accessible responsive behavior. The entry now uses a smaller mobile canvas and clearer Google/email hierarchy; install, update, and theme overlays are gated away from an unauthenticated root visit, and the install offer is delayed to fifteen seconds where eligible.
- [x] Define and obtain final approval for one short-lived managed disposable MySQL 8.4 migration-rehearsal target, including target label, preflight, sanitized evidence, and mandatory teardown; never use an existing staging or live schema. The scratch project and reported disposable database/runtime have been torn down.
- [x] Create one owner-approved empty `NSOS Migration Scratch` managed project with a fresh disposable database; do not attach a domain, provider, deployment, schedule, live secret, or copied record, and delete the project/database after evidence collection. The owner confirmed deletion after teardown.
- [x] Close the isolated-artifact retrieval path safely: the platform exposed no independent absolute path or visible scratch attachment, so no live or substitute source was used and the scratch project has been deleted.
- [x] Verify the reported disposable-database/runtime teardown and delete only owner-authorized `NSOS Migration Scratch` (`7RT4caaY8dQmJ7jCfXWRuW`) after authenticated project-management access is restored.
- [x] Verify the owner-reported removal of `NSOS Migration Scratch` (`7RT4caaY8dQmJ7jCfXWRuW`) and record that no remaining database, runtime, project, or live-system action is implied by the teardown.
- [x] Resolve or formally document the staging baseline’s reported 60 TypeScript diagnostics across 19 imported files and retained migrations `0041`–`0064 before any synthetic data setup, staging release decision, recovery rehearsal, or load probe. The isolated source-only compatibility repair formally documented the imported baseline and reported TypeScript/build success with 423 tests; this does not create or validate a runnable staging application, storage namespace, recovery, or load environment.
- [x] Diagnose and repair the tester-reported Google OAuth `redirect_uri_mismatch` at NSOS sign-in; the live client originally allowed only the managed platform callback, so the owner-approved `https://nsos.top` origin and `https://nsos.top/api/auth/google/callback` were added while retaining the managed entries. An anonymous post-save request reached Google’s account-selection page with the expected callback and no mismatch marker; no tester account, secret, tenant, domain, or data was changed.
- [x] Audit and refine the public entry, sign-in, and first-use front-end experience based on tester feedback, preserving functional role, tenant, and confirmation safeguards; validate responsive behavior before release. Reframed the public entry around truthful first-use guidance and a mobile-first operating path, retained all sign-in choices, delayed the optional PWA install prompt, verified TypeScript plus 20 focused regressions, completed a local browser review, and built production successfully. The managed screenshot endpoint was unavailable, so a tester’s normal mobile-device check remains recommended after release.
