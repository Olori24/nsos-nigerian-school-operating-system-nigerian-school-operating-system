...
| Browser and API abuse | CSP and browser-protection headers, no-store API responses, bounded request parsers, same-origin mutation guard, and route-sensitive shared database request throttling | Add a gateway/WAF for bot, volumetric, and network-layer protection before high-volume public rollout. |
| Sensitive administrator actions | Tenant-scoped append-only application audit ledger covering provider, access, domain, admissions, results, invoicing, and payments; secret/recipient metadata redaction; owner/admin-only reads | Add externally retained logs and a documented review cadence for a fuller compliance control. |
