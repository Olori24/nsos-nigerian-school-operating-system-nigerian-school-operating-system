...
import { FileUp, HandCoins, ShieldCheck, Sparkles } from "lucide-react";
...
export function FamilyCashAssurance({ schoolId, role, onDone }: { schoolId: number; role: "parent" | "student"; onDone: () => void }) {
  ...
  const submit = trpc.nsos.portal.submitPaymentEvidence.useMutation({
    onSuccess: async () => {
      ...
      setAmount(""); setPaidOn(""); setReference(""); setNote(""); setFile(null); setScanResult(null);
  ...
  const chooseFile = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    setFile(selected); setScanResult(null);
  };
  const scanReceipt = trpc.nsos.portal.scanPaymentEvidence.useMutation({
    onSuccess: result => {
      setScanResult(result);
      if (result.amountNgn) setAmount(String(result.amountNgn));
      if (result.paidOn) setPaidOn(result.paidOn);
      toast.success("Receipt scanned. Confirm or edit the suggested amount and date before sending it to finance.");
    },
    onError: error => toast.error(`${error.message} You can still enter the amount and payment date manually.`),
  });
  const startReceiptScan = async () => {
    ...
    } catch (error) { toast.error(error instanceof Error ? error.message : "The receipt scan could not be prepared."); }
  };
