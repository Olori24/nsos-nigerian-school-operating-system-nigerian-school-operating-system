...
const enrollment = { studentId: 44, biodataTransferred: true, admissionLetter: { guardianEmail: "guardian@example.com", guardianName: "Mrs. Okafor", studentName: "Amina Chiamaka Okafor", schoolName: "Greener Future Academy", schoolAddress: "Abeokuta, Ogun State", admissionNo: "GFA/2026/014", className: "Primary 1", sessionName: "2026/2027 Session", admittedOn: "2026-09-01" } };
...
describe("confirmed admission enrollment and letter delivery", () => {
  ...
  it("transfers confirmed enrollment, sends the generated letter, and records a sent delivery audit", async () => {
    await expect(caller().nsos.admissions.enrol(input)).resolves.toMatchObject({ studentId: 44, biodataTransferred: true, letterDelivery: "sent" });
    ...
    expect(db.recordSecurityAuditEvent).toHaveBeenCalledWith(expect.objectContaining({ metadata: expect.objectContaining({ biodataTransferred: true, admissionLetterDelivery: "sent" }) }));
  });
