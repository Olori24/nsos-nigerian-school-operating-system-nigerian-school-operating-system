...
export function ClearFormConfirmation({ onConfirm }: { onConfirm: () => void }) {
  ...
  return <AlertDialog open={open} onOpenChange={setOpen}><AlertDialogTrigger asChild><button type="button" className="inline-flex items-center gap-2 rounded-xl border border-[#e2cac6] bg-white px-4 py-2.5 text-sm font-bold text-[#9b3d35] transition hover:bg-[#fff7f5]"><RotateCcw className="h-4 w-4" />Clear form</button></AlertDialogTrigger><AlertDialogContent className="border-[#e2d7d3] bg-[#fffdfc]"><AlertDialogHeader><AlertDialogTitle className="text-[#542c28]">{CLEAR_FORM_TITLE}</AlertDialogTitle><AlertDialogDescription className="leading-6 text-[#735d59]">{CLEAR_FORM_DESCRIPTION}</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel className="border-[#d8ddd8]">Keep editing</AlertDialogCancel><AlertDialogAction onClick={() => { onConfirm(); setOpen(false); }} className="bg-[#9b3d35] text-white hover:bg-[#82322c]">Clear form and draft</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog>;
}
